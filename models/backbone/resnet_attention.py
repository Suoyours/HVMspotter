...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        ...
        # reduce layers
        self.reduce1_ = Conv_BN_ReLU(64,
                                     512,
                                     kernel_size=3,
                                     stride=1,
                                     padding=1)

        self.reduce2_ = Conv_BN_ReLU(128,
                                     512,
                                     kernel_size=3,
                                     stride=1,
                                     padding=1)

        self.reduce3_ = Conv_BN_ReLU(256,
                                     512,
                                     kernel_size=3,
                                     stride=1,
                                     padding=1)

        self.avgpool1 = nn.AdaptiveAvgPool2d((1, 1))  # output size = (1, 1)#平均池化
        self.fc1 = nn.Linear(2048 * block.expansion, num_classes)  # 全连接层 512*1 分类数
    ...
    def forward(self, x):
        ...
        p1 = self._upsample(f[0], f[3])
        p2 = self._upsample(f[1], f[3])
        p3 = self._upsample(f[2], f[3])
        p4 = f[3]
        ...
        r = [r1, r2, r3, p4]
        feature1 = torch.cat(r, dim=1)

        feature2 = self.ECALayer(feature1)

        x = self.avgpool1(feature2)
