...
def train(train_loader, model, optimizer, epoch, start_iter, cfg):
    ...
    losses = AverageMeter()
    losses_vertical = AverageMeter()
    losses_horizontal = AverageMeter()
    losses_two_lines = AverageMeter()
    losses_rec = AverageMeter()

    ious_vertical = AverageMeter()
    ious_horizontal = AverageMeter()
    ious_two_lines = AverageMeter()
    ...
    for iter, data in enumerate(train_loader):
        ...
        del data['gt_instances']
        del data['gt_bboxes']
        # forward
        outputs = model(**data)

        # detection loss
        loss_vertical = torch.mean(outputs['loss_vertical'])
        losses_vertical.update(loss_vertical.item())

        loss_horizontal = torch.mean(outputs['loss_horizontal'])
        losses_horizontal.update(loss_horizontal.item())

        loss_two_lines = torch.mean(outputs['loss_two_lines'])
        losses_two_lines.update(loss_two_lines.item())

        loss = loss_vertical + loss_horizontal + loss_two_lines

        iou_vertical = torch.mean(outputs['iou_vertical'])
        ious_vertical.update(iou_vertical.item())

        iou_horizontal = torch.mean(outputs['iou_horizontal'])
        ious_horizontal.update(iou_horizontal.item())

        iou_two_lines = torch.mean(outputs['iou_two_lines'])
        ious_two_lines.update(iou_two_lines.item())
        ...
        if iter % 20 == 0:
            ...
            log = f'({iter + 1}/{length}) ' \
                  ...
                  f'Loss(vertical/horizontal/two_lines/total{"/rec" if with_rec else ""}): ' \
                  f'{losses_vertical.avg:.3f}/{losses_horizontal.avg:.3f}/{losses_two_lines.avg:.3f}/{losses.avg:.3f}/' \
                  f'{"/" + format(losses_rec.avg, ".3f") if with_rec else ""} | ' \
                  f'IoU(vertical/horizontal/two_lines): {ious_vertical.avg:.3f}/{ious_horizontal.avg:.3f}/{ious_two_lines.avg:.3f}' \
...
def main(args):
    ...
    if hasattr(cfg.train_cfg, 'pretrain'):
        ...
        del checkpoint['state_dict']['module.det_head.conv2.weight']
        del checkpoint['state_dict']['module.det_head.conv2.bias']
