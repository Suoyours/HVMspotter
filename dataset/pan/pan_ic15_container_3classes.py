...
ic15_train_gt_dir = ic15_root_dir + \
                    'gt/'
# ic15_test_data_dir = ic15_root_dir + 'images_test/'
ic15_test_data_dir = ic15_root_dir + 'ploy_two_line/'
...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        if word == '#':
            ...
            words.append(word)
        if gt[-1] == '2':
            bbox = [int(gt[i]) for i in range(12)]
            bbox = np.array(bbox) / ([w * 1.0, h * 1.0] * 6)
            bboxes.append(bbox)
        else:
...
class PAN_IC15_Container_3classes(data.Dataset):
    def __init__(self,
                 split='train',
                 is_transform=False,
                 img_size=None,
    ...
    def prepare_train_data(self, index):
        ...
        gt_two_lines = np.zeros(img.shape[0:2], dtype='uint8')
        ...
        gt_two_lines = torch.from_numpy(gt_two_lines).long()
