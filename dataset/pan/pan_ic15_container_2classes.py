...
ic15_root_dir = '/data/ys/PycharmProjects2/pan_pp.pytorch/data/container/container_v1/'
...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        if word == '#1' or word == '#2' or word == '#3' or word == '3':
            words.append('###')
        else:
            words.append(word)
...
class PAN_IC15_Container_2classes(data.Dataset):
    def __init__(self,
                 split='train',
                 is_transform=False,
                 img_size=None,
    ...
    def prepare_train_data(self, index):
        ...
        if bboxes.shape[0] > 0:
            ...
            for i in range(bboxes.shape[0]):
                ...
                if words[i] == '1':
                    cv2.drawContours(gt_vertical, [bboxes[i]], -1, 1, -1)
                if words[i] == '2':
                    cv2.drawContours(gt_horizontal, [bboxes[i]], -1, 1, -1)
                if words[i] == '###':
...
if __name__ == '__main__':
    ...
    dataset_args = dict(split='train',
                        ...
                        kernel_scale=0.5,
                        read_type='cv2')
    train_data = PAN_IC15_Container_2classes(**dataset_args)
